...
12. **ระเบียบ Spatial Disambiguation (24-bit Short NodeID + H3 Cell Resolution 7/9 ⭐️):**
    - **หลักการระบุตัวตนเชิงพื้นที่ (Spatial Disambiguation Principle):**
      เพื่อรักษาขนาดของ BLE Advertising Frame ให้อยู่ภายในเพดาน **31 Bytes ของ BLE Legacy** (ไม่เพิ่มเป็น 32-bit/4B ให้บวมขึ้น +6B ใน Advertising Packet) ระบบกำหนดให้ใช้ **Short NodeID ขนาด 24-bit (3 Bytes / 16,777,216 รหัส)** ควบคู่กับเซลล์ตารางหกเหลี่ยม **H3 Cell Index (4 Bytes / Resolution 7/9)** ในการระบุตัวตนเฉพาะในระดับท้องถิ่น:
      $$\text{Unique Local Identity} = \text{H3 Cell Index (4B)} + \text{Short NodeID (3B / 24-bit)}$$
    - **การป้องกันการชนกันของรหัสในรัศมีวิทยุ (Zero Collision in Local Radio Space):**
      โอกาสที่อุปกรณ์ 2 เครื่องจะมี Short NodeID 24-bit ซ้ำกันในรัศมีเซลล์ H3 เดียวกัน (300 ม. – 5 กม.) มีความน่าจะเป็นต่ำมาก ($< 1$ ใน $10^{14}$) โดยโหนดที่มีรหัส 24-bit เดียวกันแต่อยู่ต่างอำเภอหรือต่างจังหวัด จะถูกแยกออกจากกันอย่างเด็ดขาดด้วยค่าพิกัด H3 Cell ทันที
...
- [ ] **Task 3.1: Instant Hardware-Backed Dual-Keypair Identity & Zero-Mental-Load Engine (Ed25519 + X25519 ⭐️)**
  ...
  - **Node ID Generation & Spatial Disambiguation (⭐️):** 
    - สร้าง `node_id_hash` (8 Bytes uint64) จาก Truncated SHA-256 ของ Ed25519 Public Key สำหรับ Unicast / DTN Routing
    - สกัด **Short NodeID 24-bit (3 Bytes / 16.7M IDs)** สำหรับบีบอัดลง BLE Micro-Advertising Frame (เพดาน 31B)
    - ผสานระเบียบ **Spatial Disambiguation**: ประกอบร่างตัวตนระดับท้องถิ่นด้วย `H3 Cell Index (4B) + Short NodeID (3B)` ป้องกันการชนกันของรหัสในรัศมีวิทยุ 5–10 กม. หากพบการชนกันในเซลล์เดียวกัน ให้ใช้ 2 ไบต์ท้ายของ Public Key ช่วยแยกแยะ และทำการ Silent Re-roll รหัส 24-bit อัตโนมัติในพื้นหลัง
...
- **สัญญาอนุญาตทางการ (Official Dual-Licensing Framework):**
  - **1. Open Humanitarian License:** **GNU Affero General Public License v3.0 (AGPL-3.0)**
    - อนุญาตให้ **ประชาชน ชุมชน มูลนิธิ หน่วยกู้ภัย และองค์กรสาธารณกุศลทั่วโลก ใช้งาน ดัดแปลง และเผยแพร่ได้ฟรีตลอดชีพ 100%**
    - ไม่มีโฆษณา ไม่เก็บค่าบริการ ไม่ขายข้อมูลส่วนบุคคล (Zero-Commercial Monetization for Public)
  - **2. Commercial Dual-Licensing (สงวนสิทธิ์เชิงพาณิชย์โดย Thabot):**
    ...
    - หากบริษัทเอกชนต้องการปิดบังซอร์สโค้ดหรือผูกเข้ากับบริการเชิงพาณิชย์ **จะต้องขออนุญาตเป็นลายลักษณ์อักษรและซื้อ Commercial License จาก Thabot โดยตรงเท่านั้น** (ติดต่อ `thabo47@gmail.com`) ช่วยสกัดกั้นกลุ่มทุนไม่ให้ขโมยเทคโนโลยีกู้ภัยของประชาชนไปผูกขาด

### 6.2 กลไกการระดมทุนบริจาคแบบหลายช่องทาง (Multi-Channel Donation Ingestion Matrix)
เพื่อความยั่งยืนทางการเงินของเซิร์ฟเวอร์แม่ข่ายและอุปกรณ์กระจายสัญญาณ ระบบรองรับช่องทางการรับบริจาคทั้งในระดับประเทศและสากล:

| ช่องทางการรับบริจาค | แพลตฟอร์ม / ผู้ให้บริการ | กลุ่มเป้าหมาย | ความโปร่งใส & การหักภาษี | การเชื่อมโยงอัตโนมัติ |
| :--- | :--- | :--- | :--- | :--- |
| **1. Domestic Thailand** | **PromptPay QR (สแกนพร้อมเพย์)** | ประชาชนและมูลนิธิในไทย | ค่าธรรมเนียม 0% เข้ากองทุนโดยตรง | แสดง Dynamic Slip QR บน Web Dashboard |
| **2. Global Credit Card** | **Stripe / Ko-fi / Buy Me a Coffee** | ผู้บริจาคและนักพัฒนาทั่วโลก | ตัดบัตรเครดิต Visa/Mastercard ทั่วโลก | Webhook ซิงก์ยอดเข้า Transparent Ledger |
| **3. Open Source Platform** | **Open Collective & GitHub Sponsors** | องค์กรเทคโนโลยี และชาว Open Source | มี Fiscal Host จัดการบัญชีโปร่งใส 100% | ซิงก์รายชื่อผู้สนับสนุนขึ้น Hall of Fame อัตโนมัติ |
| **4. Crypto Humanitarian** | **Bitcoin (BTC) & USDT (TRC-20)** | ผู้บริจาคที่ไม่พึ่งพาระบบธนาคาร | ไร้ตัวกลาง โอนได้แม้อยู่ในพื้นที่ปิดล้อม | ตรวจสอบผ่าน On-Chain Explorer |

### 6.3 บัญชีโปร่งใสตรวจสอบได้เรียลไทม์ (Transparent Public Expense Ledger)
- **Live Infrastructure Balance Sheet:** หน้าเว็บ `https://outgrid-rescue.pages.dev/transparency` แสดงยอดเงินบริจาคสะสมเทียบกับค่าบริการคลาวด์จริงในแต่ละเดือนแบบ Real-time
- **ประมาณการค่าใช้จ่ายจริงของระบบ (Zero-Cost Serverless Advantage):**
  - **ผู้ใช้ 100,000 คนแรก:** ค่าใช้จ่าย **$0 / เดือน (ฟรี 100% บน Cloudflare Free Tier)**
  - **ผู้ใช้ 1,000,000 คน:** ค่าใช้จ่ายประมาณ **$5 – $15 / เดือน (~175 – 500 บาท)**
  - **ผู้ใช้ระดับสากล 10,000,000 คน:** ค่าใช้จ่ายประมาณ **$40 – $80 / เดือน (~1,400 – 2,800 บาท)**
- **การจัดสรรเงินบริจาคส่วนเกิน (Hardware & Field Grants):**
  - หากมียอดเงินบริจาคเกินกว่าค่าบำรุงรักษาคลาวด์ จะนำเงิน **100% ไปจัดซื้อฮาร์ดแวร์จริง:**
    1. จัดซื้อและประกอบบอร์ด **ESP32 Solar Repeater (กล่องทวนสัญญาณพลังงานแสงอาทิตย์)** เพื่อนำไปติดตั้งบนยอดเขา ยอดเสา หรือหลังคาวัดในพื้นที่เสี่ยงภัยน้ำท่วม/ดินถล่มซ้ำซาก
    2. จัดทำ **Emergency Sideload Flash Drives (USB Bootable & APK Preloaded)** แจกจ่ายให้ทีมกู้ภัยอาสาประจำตำบล

### 6.4 การกระจายภาระแม่ข่ายระดับชุมชน (P2P Community Federation & Self-Hosting)
- **Zero Central Dependency:** นอกเหนือจากเซิร์ฟเวอร์ส่วนกลางของโปรเจกต์ โค้ด Backend ทั้งหมด (Cloudflare Workers + D1 Schema) ถูกเปิดเป็น Open Source 100%
- **Local Municipality Federation:** เปิดให้มหาวิทยาลัย, องค์การบริหารส่วนตำบล (อบต.), เทศบาล หรือสมาคมวิทยุสมัครเล่น สามารถกด Clone และ Deploy เซิร์ฟเวอร์สำรองประจำจังหวัดของตนเองได้ภายใน 5 นาที เพื่อให้โครงข่ายระดับท้องถิ่นสามารถดำรงอยู่ได้แม้สัญญาณสูญหายจากส่วนกลาง

### 6.5 โครงการความปลอดภัยและการมีส่วนร่วมของชุมชน (Bug Bounty & Governance)
- **Responsible Vulnerability Disclosure:** มีข้อกำหนดเปิดรับรายงานช่องโหว่ความปลอดภัยผ่าน `thabo47@gmail.com` พร้อมรางวัลตอบแทนจากกองทุนบริจาค (Humanitarian Bug Bounty)
- **Community Code Review:** เชิญชวนผู้เชี่ยวชาญด้าน Cryptography ทั่วโลกตรวจสอบความถูกต้องของอัลกอริทึม Ed25519 และ E2EE อย่างต่อเนื่อง เพื่อรับประกันความปลอดภัยของประชาชนสูงสุด
...
เพื่อให้การพัฒนา การทดสอบ และการส่งมอบซอฟต์แวร์เป็นไปตามมาตรฐานสูงสุด ปลอดภัย ไร้ข้อผิดพลาด และสอดคล้องกับกฎระเบียบของโครงการอย่างเคร่งครัด จึงกำหนดข้อบังคับการปฏิบัติงานดังต่อไปนี้:

1. **นโยบายการจัดการสาขา Git (Git Branch & Promotion Policy):**
   - การพัฒนาโค้ดและชุดทดสอบทั้งหมด **ต้อง Commit และ Push ไปยัง branch `uat` เท่านั้น**
   - **ห้าม Push ไปยัง branch `main` โดยเด็ดขาด** การรวมโค้ดเข้าสู่ `main` จะทำได้ผ่านการ Fast-forward Merge หลังจากผ่านการทดสอบคุณภาพและการยอมรับบน UAT ครบถ้วน 100% แล้วเท่านั้น
2. **การรักษาความสมบูรณ์ของหน้าจอและฟังก์ชัน (UI & Feature Integrity):**
   - **ห้ามลบ ซ่อน หรือตัดปุ่ม ลิงก์ เมนูนำทาง หรือฟังก์ชันการทำงานเดิมที่มีอยู่แล้วออกโดยเด็ดขาด** เว้นแต่จะมีคำสั่งเป็นลายลักษณ์อักษรจากผู้ใช้โดยตรง
   - รักษาความสมบูรณ์ในการเข้าถึง (100% Functional Parity) สำหรับทั้ง **ผู้ใช้ทั่วไป (Guest)** และ **ผู้ใช้ที่ยืนยันตัวตนแล้ว (Authenticated/Responder)** โดยฟังก์ชันฉุกเฉิน (One-Tap SOS, แผนที่ออฟไลน์, แชต 1-on-1) ต้องพร้อมใช้งานทันทีแบบ Zero-Mental-Load
3. **การตรวจสอบไวยากรณ์และความปลอดภัยก่อนการส่งมอบ (Syntax & Quality Gates):**
   - ทุกครั้งก่อนทำการ Commit จะต้องรันการตรวจสอบไวยากรณ์โค้ดผ่านคำสั่ง:
     ```bash
     node scripts/checkSyntax.js
     ```
   - ยืนยันว่าชุดทดสอบอัตโนมัติ (Automated Unit Tests ทั้ง 51 ชุด) ผ่านการทดสอบ 100% (Zero Test Failure)
4. **ความยินยอมของผู้ใช้ก่อนการแก้ไขหรือเขียนทับไฟล์ (File Mutation & Overwrite Consent):**
   - ห้ามแก้ไขไฟล์ใดๆ โดยไม่ได้รับการอนุญาตหรือคำสั่งที่ชัดเจนจากผู้ใช้
   - หากมีความจำเป็นต้องลบไฟล์ (Delete) หรือเขียนทับเนื้อหาทั้งไฟล์ (Overwrite / Replace Entirely) **ต้องแจ้งเตือนให้ผู้ใช้ทราบและขออนุญาตก่อนเสมอ**
5. **การตรวจสอบผลกระทบแบบครบวงจร (End-to-End Dependency & System Impact Verification):**
   - ต้องตรวจสอบผลกระทบข้างเคียงและจุดเชื่อมโยงทั้งหมด (End-to-End Impact) ทั้งในการวางแผน (Planning) และการลงมือเขียนโค้ด (Execution)
   - ห้ามมองข้ามระบบเบื้องหลัง เช่น Webhook, Cloudflare D1 Schema, Public STUN, Rate Limiter หรือ Third-party Bridge (CAP v1.2, Meshtastic, Briar)
6. **การส่งมอบงานแบบครอบคลุมและลดการโต้ตอบยิบย่อย (Comprehensive Single-Turn Execution):**
   - ปฏิบัติตามแนวทางแก้ไขครอบคลุมหลายไฟล์ในเทิร์นเดียวอย่างเป็นระบบ
   - วางโครงสร้างแผนงานให้ชัดเจน ดำเนินการขั้นตอนทางเทคนิคให้เสร็จสิ้น แล้วจึงส่งมอบผลการดำเนินงานให้ผู้ใช้ตรวจทาน
7. **การรองรับสภาพแวดล้อมคู่ขนาน (Dual-Environment Parity Support):**
   - ซอร์สโค้ด, ไฟล์คอนฟิก (`wrangler.toml`), สคริปต์ และ CI/CD Workflow ต้องรองรับการแยกตัวแปรสภาพแวดล้อมระหว่าง **`uat` (Staging/Testing)** และ **`main` (Production Live)** อย่างชัดเจนและแยกฐานข้อมูลออกจากกันเด็ดขาด
8. **การสงวนสิทธิ์ทางปัญญาและลิขสิทธิ์ (Attribution & Intellectual Property Compliance):**
   - ต้องคงข้อความลิขสิทธิ์ สัญญาอนุญาต **AGPL-3.0 + Commercial Dual-Licensing** และชื่อผู้คิดค้น/สถาปนิกหลัก **Thabot** (<thabo47@gmail.com>) และโพรโทคอล **Thabot OutGrid Protocol (TOG v1.1)** ไว้ในส่วนหัวของไฟล์ซอร์สโค้ดและเอกสารสำคัญทุกฉบับอย่างสมบูรณ์
9. **นโยบายความปลอดภัย Zero-Storage & Data Privacy:**
   - ห้ามออกแบบหรือเขียนโค้ดเพื่อจัดเก็บข้อความแชตส่วนบุคคลบนคลาวด์เด็ดขาด (Zero Message Cloud Storage)
   - ข้อมูลพิกัดบนคลาวด์สาธารณะต้องผ่านการเบลอเชิงพื้นที่ตามมาตรฐาน K-Anonymity และ Uber H3 Res 7 เสมอ
...
### 9.3 สถาปัตยกรรมเครือข่ายกู้ภัย 2 ระดับ: In-City Mobile Mesh + Inter-City LoRa Backbone (⭐️ 2-Tier Hierarchical Disaster Network)
เพื่อแก้ปัญหาคอขวดของการส่งข้อมูลระยะไกลข้ามอำเภอ/ข้ามจังหวัด โดยที่ **ประชาชนในเมืองยังคงไม่ต้องซื้อฮาร์ดแวร์ใดๆ เพิ่มแม้แต่บาทเดียว (Zero Hardware Barrier for Citizens)** ระบบได้กำหนดโครงสร้างเครือข่ายแบบ 2 ลำดับชั้น (Hierarchical Hybrid Topology) ดังนี้:

#### 1. แผนผังการทำงานของโครงข่าย 2 ระดับ (2-Tier Architecture Topology)
```text
  [ อำเภอ ก / ตัวเมืองชั้นใน ]                                      [ อำเภอ ข / ศูนย์บัญชาการกู้ภัยหลัก ]
  ┌────────────────────────────────────────┐                       ┌────────────────────────────────────────┐
  │  ประชาชน & ทีมกู้ภัยในเมือง            │                       │  ศูนย์รับแจ้งเหตุ & โรงพยาบาลสนาม      │
  │  [📱 Citizen A]                        │                       │  [📱 Rescue Base Commander]            │
  │       ▲ (BLE / Wi-Fi Mesh)             │                       │       ▲ (BLE / Wi-Fi Mesh)             │
  │       ▼                                │                       │       ▼                                │
  │  [📱 Citizen B] ◄──► [📱 Citizen C]     │                       │  [📱 Field Medic] ◄──► [📱 Police]      │
  └──────────────────┬─────────────────────┘                       └───────────────────▲────────────────────┘
                     │ (BLE Long Range: Free)                                          │ (BLE Long Range)
                     ▼                                                                 ▼
         ┌───────────────────────┐                                         ┌───────────────────────┐
         │ 🗼 LoRa Bridge Node A  │ ═══════════════════════════════════════ │ 🗼 LoRa Bridge Node B  │
         │ (ESP32 + SX1262 LoRa) │       Long-Range LoRa Backbone          │ (ESP32 + SX1262 LoRa) │
         │ แขวนบนยอดตึก/เสาสูง   │        (AS923 MHz: 15 – 35 km)          │ แขวนบนยอดเขา/ศูนย์สั่งการ│
         │ [Solar + 18650 Auto]  │ ═══════════════════════════════════════ │ [Solar + 18650 Auto]  │
         └───────────────────────┘                                         └───────────────────────┘
```

#### 2. ข้อกำหนดการทำงานรายระดับ (Tier Functional Separation)
1. **Tier 1: In-City Mobile Mesh (เครือข่ายฝูงชนภายในเมือง):**
   - **อุปกรณ์:** สมาร์ตโฟนทั่วไป 100% (Android & iOS) ติดตั้งแอป OutGrid Mesh
   - **ตัวกลางเชื่อมต่อ:** Bluetooth 5 LE Coded PHY (S=8 / S=2) และ Wi-Fi Direct P2P (ความถี่ 2.4 GHz และ 5 GHz)
   - **หน้าที่:** รองรับการสื่อสารรอบด้านในระยะ 200–400 เมตร/ทอด ส่งข้อความแชต 1-on-1, ภาพถ่ายสถานการณ์ WebP, และคลิปเสียงสั้น Opus 15 วิ ได้อย่างอิสระ ไม่ต้องมีเราเตอร์
2. **Tier 2: Inter-City Long-Range Backbone (แกนส่งต่อระยะไกลข้ามเมือง):**
   - **อุปกรณ์:** บอร์ดไมโครคอนโทรลเลอร์อิสระ **ESP32 + Semtech SX1262 LoRa Transceiver** (ต้นทุนต่ำ ~850–1,200 บาท/จุด) พร้อมชุดโซลาร์เซลล์และแบตเตอรี่ 18650 ติดตั้งเพียง 1–2 จุดต่อตำบล/เทศบาล (บนหอแท็งก์น้ำ, หลังคาวัด, ยอดเสาสูง หรือยอดเขา)
   - **ตัวกลางเชื่อมต่อ:** คลื่นวิทยุกำลังส่งต่ำย่านความถี่ LoRa AS923 (920–925 MHz สำหรับประเทศไทยและเอเชีย) หรือ 433/868 MHz ยิงสัญญาณทะลุป่าทึบและภูเขาได้ไกล **15 – 35+ กิโลเมตรต่อทอด**
   - **นโยบายคัดกรองข้อมูลฉุกเฉิน (Strict Priority Traffic Filtering):** เพื่อป้องกันแบนด์วิดท์ LoRa (ซึ่งมีความเร็วต่ำเพียง 1–5 kbps) อุดตัน บอร์ด Bridge Node จะคัดกรองและส่งต่อเฉพาะเฟรมสำคัญวิกฤตเท่านั้น:
     - ✅ **`0x01: SOS_BEACON` (ขนาด 21 Bytes):** พิกัด ละติจูด/ลองจิจูด + H3 Index + สถานะผู้ประสบภัย (พิกัดแม่นยำระดับ <1 เมตร ด้วย H3 Delta Compression)
     - ✅ **`0x04: CRISIS_FEED` (ขนาด $\le 128$ Bytes):** ข้อความแจ้งเตือนภัยพิบัติฉุกเฉินระดับชาติพร้อมลายเซ็น Ed25519
     - ✅ **`0x05: DELIVERY_ACK` (ขนาด 10 Bytes):** ใบเสร็จยืนยันว่าผู้ประสบภัยได้รับการช่วยเหลือแล้ว
     - 🚫 **ภาพถ่าย WebP และไฟล์เสียง Opus จะถูกระงับไม่ให้ขึ้นช่อง LoRa เด็ดขาด** (ให้ส่งผ่านมือถือ Tier 1 หรือ Data Mule รถกู้ภัยเท่านั้น)
3. **การแยกช่องความถี่ไร้การรบกวน (Zero RF Interference Guarantee):**
